...
def getvideofromimage(image_path, text, index):
    ...
    video_file = f'output_video_{index}.mp4'
    ffmpeg.input(image_path, loop=1, t=duration_seconds).output(
        video_file, r=24, pix_fmt='yuv420p', vcodec='libx264'
    ).run()

    # Merge video and audio
    outputvideoaudio_path = f'output_video_with_audio{index}.mp4'
    ffmpeg.input(video_file).input(output_file).output(
        outputvideoaudio_path, vcodec='copy', acodec='aac', audio_bitrate='192k', strict='experimental'
    ).run()

    return outputvideoaudio_path

def add_subtitles_to_video(video_path, subtitle_path, index):
    output_video_file = f'output_video_subtitling{index}.mp4'
    ffmpeg.input(video_path).output(
        output_video_file, vf=f"subtitles={subtitle_path}"
    ).run()
    return output_video_file
...
def main():
    ...
    if uploaded_files:
        ...
        step = st.sidebar.text_area("Write steps in the video")

        changes = st.sidebar.text_area("Changes in video")

        result_text = st.sidebar.text_area("Result Text")
        ...
        Fix = st.sidebar.text_area("Fix the Spelling")
        if st.sidebar.button("Submit", key='sidebutton2'):
            ...
            for i in range(min(len(video_paths), len(sub_paths))):
                sub_video_paths.append(add_subtitles_to_video(video_paths[i], sub_paths[i], i))
            print(sub_video_paths)
            
            # Concatenate videos using ffmpeg
            concat_file = "concat_list3.txt"
            with open(concat_file, 'w') as f:
                for video_file in sub_video_paths:
                    f.write(f"file '{video_file}'\n")
